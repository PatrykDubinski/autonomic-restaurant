class RestaurantGuestsController < ApplicationController
  ...
  def index
    # @restaurant_guests = RestaurantGuest.all
    byebug
    @restaurant_guest = RestaurantGuest.new

    @restaurant_guest.code = SecureRandom.uuid

    # segment = RQRCodeCore::QRSegment.new(data: @restaurant_guest.code.to_s)
    #
    # qrcode = RQRCodeCore::QRCode.new(segment)
    ...
    unless self.image.attached?
      @restaurant_guest.image.attach(png)
    end
    if @restaurant_guest.save
      render :guest_code, status: :created
    end
  end
